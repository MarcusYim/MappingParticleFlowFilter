...
al_inv_prior_cov = np.linalg.inv(0.1 * prior_cov)
...
def flow_matrix(xs, y, D):
    return [0.05 * sum_flow_eq(xs[i], xs, y, D) for i in range(xs.shape[0])]
...
while np.max(flows) > 0.01:
    flows = flow_matrix(xs_pff, y, D)
    xs_pff = xs_pff + flows

print(np.average(xs_pff, 0))
print(xs_pff)
...
plt.axhline(y = kf.x[0], color = 'b', linestyle = '-')
plt.axvline(x = kf.x[1], color = 'b', linestyle = '-')
